...
import { columnSettingsSchema, LOGBOOK_NAME_MAX, toLogbookDetail } from "./utils.ts";
...
export const logbookRouter = router({
  ...
  create: publicAuthoringProcedure
    ...
    .mutation(async ({ ctx, input }): Promise<CreatedLogbook> => {
      ...
      const lb = ctx.em.create(Logbook, { name, columns: [] });
      ctx.em.persist(lb);
      await ctx.em.flush();
  ...
  /**
   * Replace the logbook's org-view column settings (names + widths) wholesale.
   * The frontend owns the full set, so we swap the embedded array out rather
   * than diffing. Input validity (bounded size, unique column numbers) is
   * enforced by `columnSettingsSchema`.
   */
  setColumns: logbookProcedure
    .input(z.object({ columns: columnSettingsSchema }))
    .mutation(async ({ ctx, input }): Promise<LogbookDetail> => {
      const lb = ctx.logbook;
      // `em.assign` hydrates the plain objects into ColumnSetting embeddables and
      // marks the embedded array dirty; a plain reassignment wouldn't type-check
      // against the embeddable class.
      ctx.em.assign(lb, {
        columns: input.columns.map((c) => ({ col: c.col, name: c.name, wide: c.wide })),
      });
      await ctx.em.flush();
      return toLogbookDetail(lb);
    }),
